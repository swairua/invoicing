...
function getTableDefinitions() {
    return [
        ...
        'invoices' => 'id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, company_id INT NOT NULL, customer_id INT NOT NULL, quotation_id INT, invoice_number VARCHAR(100) NOT NULL UNIQUE, invoice_date DATE, due_date DATE, subtotal DECIMAL(15,2) DEFAULT 0, tax_amount DECIMAL(15,2) DEFAULT 0, total_amount DECIMAL(15,2) DEFAULT 0, paid_amount DECIMAL(15,2) DEFAULT 0, balance_due DECIMAL(15,2) DEFAULT 0, notes TEXT, terms_and_conditions TEXT, lpo_number VARCHAR(255), status VARCHAR(50) DEFAULT "draft", created_by INT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, UNIQUE KEY invoice_number (invoice_number), KEY fk_invoices_customer (customer_id), INDEX idx_invoices_status (status)',
