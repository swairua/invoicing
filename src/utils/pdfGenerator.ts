  ...
  const htmlContent = `
    ...
    <head>
      ...
      <style>
        ...
        * {
        ...
        ${getTemplateCSS(templateName, primaryColor)}
    ...
    <body>
      <div class="page">
        ...
        <!-- Header Section - Generated by Template System -->
        ${renderHeaderHTML(company as any, {
          documentTitle,
          ...
          additionalDetails: [
            ...
            {
              label: data.type === 'receipt' ? 'Amount Paid' : data.type === 'remittance' ? 'Total Payment' : data.type === 'lpo' ? 'Order Total' : 'Amount',
              value: formatCurrency(data.total_amount)
            }
          ]
        }, templateName, primaryColor)}
...
export const generateCustomerStatementPDF = async (customer: any, invoices: any[], payments: any[], creditNotes: any[] = [], statementData?: any, company?: CompanyDetails) => {
  ...
  const allTransactions = [
    ...
    // Add all credit notes as credits
    ...creditNotes.map(cn => ({
      date: cn.credit_note_date,
      type: 'credit_note',
      reference: cn.credit_note_number || cn.id || 'CN',
