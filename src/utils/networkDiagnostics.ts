...
export async function testDNSResolution(): Promise<DiagnosticResult> {
  try {
    console.log('🔍 Testing DNS resolution for diagsolutionsltd.com');

    // Use public DNS API
    const response = await fetch('https://dns.google/resolve?name=diagsolutionsltd.com', {
    ...
    if (response.ok) {
      ...
      return {
        ...
        details: { domain: 'diagsolutionsltd.com', hasAnswers },
      };
    } else {
      return {
        test: 'DNS Resolution',
...
export function getTroubleshootingSuggestions(errorType: string): string[] {
  const suggestions: Record<string, string[]> = {
    failed_to_fetch: [
      ...
      '6. Verify backend server is running at https://diagsolutionsltd.com',
    ],
    timeout: [
